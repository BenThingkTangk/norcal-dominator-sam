"""NorCal Insurance Dominator — Vercel Serverless FastAPI Backend (Streaming)."""
...
def stream_claude(system: str, messages: list, max_tokens: int = 2048):
    def generate():
        try:
            from anthropic import Anthropic
            api_key = os.environ.get("ANTHROPIC_API_KEY")
            if not api_key:
                yield f"data: {json.dumps({'text': 'Error: ANTHROPIC_API_KEY not set'})}\n\n"
                yield "data: [DONE]\n\n"
                return
            ...
            for model in MODELS:
                try:
                    ...
                    last_err = str(e)
                    if "credit" not in last_err.lower() and "model" not in last_err.lower():
                        break
                    continue
            yield f"data: {json.dumps({'text': f'Error: {last_err}'})}\n\n"
            yield "data: [DONE]\n\n"
        except Exception as e:
...
async def chat(req: ChatRequest):
    ...
    if req.system_context:
        system += "\n\nADDITIONAL CONTEXT:\n" + req.system_context
    return stream_claude(system, req.messages)
...
Generate with sections: 1. OPENING HOOK (2 sentences max) 2. PAIN AMPLIFICATION (with real data) 3. THE BRIDGE 4. SOCIAL PROOF 5. CALL TO ACTION 6. EQ NOTES. Be specific, use real numbers."""
    return stream_claude(SYSTEM_PROMPT_BASE, [{"role": "user", "content": prompt}])

@app.post("/api/objection")
async def objection(req: ObjectionRequest):
    prompt = f"""Prospect objection during recruiting call:

OBJECTION: "{req.objection}"
{"CONTEXT: " + req.prospect_context if req.prospect_context else ""}

Respond with: 1. ACKNOWLEDGE (1 empathetic sentence) 2. REFRAME (2-3 sentences with real CA insurance data) 3. WORD-FOR-WORD REBUTTAL (in quotes, conversational) 4. FOLLOW-UP QUESTION 5. EMOTIONAL READ 6. IF THEY PUSH BACK AGAIN. Be tactical and direct."""
    return stream_claude(SYSTEM_PROMPT_BASE, [{"role": "user", "content": prompt}], 1500)
    ...
    if req.carrier:
        for k, v in CARRIER_INTEL.items():
            if k.lower() in req.carrier.lower():
                ...
                break
    query = req.query or f"Full intelligence briefing on {req.county or req.carrier or 'NorCal market'}"
    system = SYSTEM_PROMPT_BASE + ("\n\nRELEVANT DATA:\n" + "\n".join(ctx) if ctx else "")
    return stream_claude(system, [{"role": "user", "content": query}])
